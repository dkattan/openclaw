...
export function createBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "OpenClaw's built-in browser control path is disabled on this host; do not suggest or use the isolated OpenClaw-managed browser.",
      "For browser work here, use the separately configured Playwright MCP extension bridge, which attaches to Darren's existing browser session after initial setup.",
      "Do not suggest or use Chrome MCP existing-session/user profiles unless they are explicitly configured; those paths require visible browser consent in this environment.",
      "Do not use profile=\"openclaw\" or profile=\"user\" here unless Darren explicitly re-enables that browser path.",
