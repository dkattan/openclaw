...
export function createIMessageProgressBubble(params: ProgressBubbleParams): IMessageProgressBubble {
  ...
  const getClient = async (): Promise<IMessageRpcClient> => {
    ...
    try {
      ...
      // A failed spawn must not poison the cached promise forever: drop it so
      // the next update retries with a fresh client.
      clientPromise = undefined;
      throw err;
    }
  };

  /** Stops the cached client and clears it so the next edit spawns a fresh
   *  one. Used after a transport failure: the wedged child would otherwise
   *  stay cached and every later edit fails against a dead bridge (each
   *  rotation meanwhile spawning a new per-send client, the 2026-09-18
   *  bridge-wedge process leak). */
  const recycleClient = async (): Promise<void> => {
    const stalePromise = clientPromise;
    clientPromise = undefined;
    if (!stalePromise) {
      return;
    }
    try {
      ...
      // A failed spawn has no child to stop.
    }
  };
  ...
  const update = async (text: string): Promise<void> => {
    ...
    const run = chain.then(async () => {
      ...
      try {
        ...
        // must not wedge the turn: fall back to a fresh bubble. The cached
        // RPC client is recycled first — a bridge stall kills the shared
        // child, and keeping it cached made every later rotation fail while
        // its per-send fallback kept spawning replacements (process leak).
        await recycleClient();
  ...
  const dispose = async (): Promise<void> => {
    ...
    // Stop the cached rpc child; leaving it cached leaked one imsg rpc
    // process per narrated turn (the accumulated imsg rpc processes).
    await recycleClient();
  };
