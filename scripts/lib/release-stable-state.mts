...
// Older writers recorded the retired parentApprovalReceipt capability; discard it on read.
const releaseStateReadSchema = releaseStateSchema.extend({
  capabilities: releaseStateSchema.shape.capabilities
    .unwrap()
    .extend({ parentApprovalReceipt: z.boolean().optional() })
    .transform(({ parentApprovalReceipt: _parentApprovalReceipt, ...capabilities }) => capabilities)
    .optional(),
});
